                                ...
                                <div class="flex flex-col items-end gap-2">
                                    @if($item->returnableQuantity() > 0)
                                        <form action="{{ route('orders.return', $order) }}" method="POST" class="js-confirm"
                                              ...
                                              data-confirm-message="Return this item? This can't be undone."
                                              data-confirm-label="Return"
                                              data-confirm-variant="warning">
                                            @csrf
                                            <input type="hidden" name="items[0][order_item_id]" value="{{ $item->id }}">
                                            <input type="hidden" name="items[0][quantity]" value="{{ $item->returnableQuantity() }}">
                                            <button type="submit"
                                    ...
                                    @endif
                                    @if($item->returned_quantity > 0)
                                        <form action="{{ route('orders.return.undo', $order) }}" method="POST" class="js-confirm"
                                              data-confirm-title="Cancel return?"
                                              data-confirm-message="Cancel the return on this item? It will be part of the order again."
                                              data-confirm-label="Cancel Return"
                                              data-confirm-variant="warning">
                                            @csrf
                                            <input type="hidden" name="items[0][order_item_id]" value="{{ $item->id }}">
                                            <input type="hidden" name="items[0][quantity]" value="{{ $item->returned_quantity }}">
                                            <button type="submit"
